...
struct LoginResponse: Decodable {
    let token: String
    let type: String
  // Vous pouvez ajouter d'autres champs renvoyés (userID, email, role, etc.)
}
...
struct RegisterResponse: Decodable {
    let token: String
    let type: String
  // Parfois l'API renvoie directement un token ici aussi. Adaptez selon votre backend.
}
...
struct RegisterRequest: Encodable {
    ...
    let lastname: String
  // Ajoutez d'autres champs si nécessaire : nom, prénom, etc.
}
...
actor APIClient {
  ...
  func register(email: String, password: String, firstname: String, lastname: String) async throws -> RegisterResponse {
    guard let url = URL(string: "/register", relativeTo: baseURL) else {
      throw APIError.invalidURL
    }
